...
*The subclass design assumes every duck of the same type flies and quacks the same way. If one duck needs different behavior, it may inherit something it shouldn’t, and changing the behavior means editing duck classes instead of swapping it for that individual duck.*
